 ...
 *
 * Read-only tests discover the current seeded competition and Demo User through
 * /api/competitions/demo-context.
 ...
 * Usage:
 *   cd backend
 *   MONGO_URI="..." node test/edge-cases.js
 *
...
async function run() {
  ...
  const demoContext = await api.get('/competitions/demo-context');
  if (demoContext.status !== 200 || !demoContext.body?.data) {
    throw new Error('Demo data is unavailable. Run npm run seed before this test.');
  }
  competitionId = demoContext.body.data.competitionId;
  userId = demoContext.body.data.demoUserId;
  ...
  {
    const r = await api.get(`/competitions/${competitionId}`);
  ...
  {
    const r = await api.get(`/competitions/${competitionId}/winners`);
    assert(r.status === 200, 'Valid competition → 200', `Got ${r.status}`);
  ...
  {
    const r = await api.get(`/competitions/${competitionId}/registration-status/${userId}`);
  ...
  {
    const r = await api.get(`/competitions/${competitionId}/registration-status/${FAKE_OID}`);
    assert(r.status === 404 || r.status === 200, 'Unknown user → 404 or handled', `Got ${r.status}`);
  }
  {
    const r = await api.get(`/competitions/${competitionId}/registration-status/${INVALID_ID}`);
    assert(r.status === 400, 'Invalid user ID → 400', `Got ${r.status}`);
  }
  {
    const r = await api.get(`/competitions/${FAKE_OID}/registration-status/${userId}`);
    assert(r.status === 404, 'Unknown competition → 404', `Got ${r.status}`);
  }
  ...
  {
    const r = await api.post(`/competitions/${competitionId}/register`, {});
    assert(r.status >= 400, 'Empty body → 4xx', `Got ${r.status}`);
  }
  {
    const r = await api.post(`/competitions/${competitionId}/register`, { userId: INVALID_ID });
    assert(r.status === 400, 'Invalid userId → 400', `Got ${r.status}`);
  }
  {
    const r = await api.post(`/competitions/${competitionId}/register`, { userId: FAKE_OID });
    assert(r.status === 404, 'Non-existent userId → 404', `Got ${r.status}`);
  }
  {
    const r = await api.post(`/competitions/${FAKE_OID}/register`, { userId });
    assert(r.status === 404, 'Non-existent competition → 404', `Got ${r.status}`);
  }
  {
    const r = await api.post(`/competitions/${INVALID_ID}/register`, { userId });
